...
with tf.Session() as sess:
    ...
    one_hot_labels0 = tf.one_hot(
        indices=tf.cast(y0, tf.int32), depth=CHAR_SET_LEN)
    one_hot_labels1 = tf.one_hot(
        indices=tf.cast(y1, tf.int32), depth=CHAR_SET_LEN)
    one_hot_labels2 = tf.one_hot(
        indices=tf.cast(y2, tf.int32), depth=CHAR_SET_LEN)
    one_hot_labels3 = tf.one_hot(
        indices=tf.cast(y3, tf.int32), depth=CHAR_SET_LEN)
    ...
    for i in range(6001):
        ...
        if i % 20 == 0:
            ...
            if i == 6000:
                saver.save(
                    sess, "captcha/models/crack_captcha.model", global_step=i)
                break
    coord.request_stop()
    coord.join(threads)
